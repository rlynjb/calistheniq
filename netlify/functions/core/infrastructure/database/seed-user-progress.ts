...
export async function seedUserProgress() {
  try {
    ...
    const categoryMap: Record<string, string> = Object.fromEntries(categories.map(c => [c.name, c.id]));
    const levelMap: Record<number, string> = Object.fromEntries(levels.map(l => [l.level_order, l.id]));
    ...
    for (const [category, level] of Object.entries(mockCurrentUserLevels)) {
      try {
        await sql`
          ...
          VALUES (${user.id}, ${categoryMap[category]}, ${level}, ${levelMap[level as unknown as number]})
